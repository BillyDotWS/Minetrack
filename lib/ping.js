...
// Player-hosted servers (MPS) — only servers with a host attached, from /v1/game-servers/mps
let _mpsCachePromise = null
let _mpsCacheTime = 0

function fetchMps (timeout) {
  const baseUrl = process.env.MINEPLEX_API_BASE_URL || config.mineplexApi.baseUrl
  const apiKey = process.env.MINEPLEX_API_KEY || config.mineplexApi.apiKey
  const url = new URL('/v1/game-servers/mps?perPage=200', baseUrl)
  const http = url.protocol === 'https:' ? require('https') : require('http')
    ...
    const req = http.get(url, { headers }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          reject(new Error('Invalid JSON from game-servers/mps API'))
        }
      })
    })
    req.on('error', reject)
    setTimeout(() => { req.destroy(); reject(new Error('API request timed out')) }, timeout)
  })
}

function getCachedMps (timeout) {
  const now = Date.now()
  if (_mpsCachePromise && (now - _mpsCacheTime) < config.rates.pingAll) {
    return _mpsCachePromise
  }
  _mpsCacheTime = now
  _mpsCachePromise = fetchMps(timeout).catch(err => {
    _mpsCachePromise = null
    throw err
  })
  return _mpsCachePromise
}
...
function ping (serverRegistration, timeout, callback, version) {
  const data = serverRegistration.data

  // Player Servers tile: count only host-attached MPS servers (sum their player counts)
  const source = data.mpsAggregate
    ? getCachedMps(timeout).then(mps => (mps.entries || []).reduce((sum, e) => sum + (typeof e.players === 'number' ? e.players : 0), 0))
    : getCachedCounts(timeout).then(counts => counts[data.gameType])

  source
    .then(playerCount => {
      callback(null, {
        players: {
          online: typeof playerCount === 'number' ? capPlayerCount(data.ip, playerCount) : 0
        }
      })
    })

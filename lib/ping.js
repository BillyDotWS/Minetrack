...
function ping (serverRegistration, timeout, callback, version) {
  getCachedCounts(timeout)
    .then(counts => {
      ...
      let playerCount
      if (Array.isArray(data.aggregateGameTypes)) {
        // Combined tile (e.g. Player Servers) — sum the counts of every member gameType
        playerCount = data.aggregateGameTypes.reduce((sum, type) => sum + (typeof counts[type] === 'number' ? counts[type] : 0), 0)
      } else {
        playerCount = counts[data.gameType]
      }

...
class Server {
  ...
  fetchGameServers (gameType) {
    ...
    const url = new URL(`/v1/game-servers/${gameType}?perPage=200`, baseUrl)
  ...
  handleGameserversRequest (res, gameType) {
    // gameType "mps" hits /v1/game-servers/mps, which lists only host-attached player servers
    this.fetchGameServers(gameType)
      .then(page => {
        res.writeHead(200, {

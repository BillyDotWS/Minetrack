...
const GAMESERVERS_URL_REGEX = /^\/api\/gameservers\/([a-z0-9-]+)$/

// The Player Servers tile (gameType "mps") aggregates every MPS source
const MPS_AGGREGATE_GAME_TYPES = ['mps-legacy', 'mps-modern', 'mps-nano', 'mps']
...
class Server {
  ...
  createHttpServer () {
    ...
    this._http = http.createServer((req, res) => {
      ...
      // Handle /api/gameservers/:gameType proxy requests
      const gameserversMatch = req.url.match(GAMESERVERS_URL_REGEX)
      if (gameserversMatch) {
        this.handleGameserversRequest(res, gameserversMatch[1])
        return
      }
  ...
  // Fetch the live server list for a single game type from the Mineplex API.
  // Resolves to a GameServerPageResponse ({ entries: [{ friendlyName, players, state }] }).
  fetchGameServers (gameType) {
    ...
    const url = new URL(`/v1/game-servers/${gameType}`, baseUrl)
    ...
    return new Promise((resolve, reject) => {
      httpModule.get(url, { headers }, (apiRes) => {
        let body = ''
        ...
        apiRes.on('end', () => {
          try {
            ...
            reject(new Error('Invalid JSON from game-servers API'))
          }
        })
      }).on('error', reject)
    })
  ...
  handleGameserversRequest (res, gameType) {
    // The Player Servers tile aggregates every MPS source into one combined list
    const gameTypes = gameType === 'mps' ? MPS_AGGREGATE_GAME_TYPES : [gameType]

    Promise.all(gameTypes.map(type => this.fetchGameServers(type).catch(() => ({ entries: [] }))))
      .then(pages => {
        ...
        res.end(JSON.stringify({ page: 1, perPage: entries.length, totalEntries: entries.length, entries }))
      })
      .catch((err) => {
        logger.log('error', 'Gameservers proxy error: %s', err.message)
        res.writeHead(502, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({ error: 'Failed to fetch game servers' }))
      })
  }
